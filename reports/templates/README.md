...
| [`EVIDENCE_CARD_TEMPLATE.md`](EVIDENCE_CARD_TEMPLATE.md) | Optional working aid · أداة عمل اختيارية | A reusable card for structuring claims inside the required reports. · بطاقة قابلة لإعادة الاستخدام لتنظيم الادعاءات داخل التقارير الإلزامية. |
...
1. Work from your saved Drive notebook. `C23` and `C28` generate the two required learner reports automatically. · اعمل من نسخة Drive المحفوظة؛ تولد `C23` و`C28` تقريري المتدرب الإلزاميين تلقائيًا.
...
6. Run `C29_EXPORT_SAFETY_CHECK`, download the completed notebook separately, and upload only the combined clean contents to the empty public repository. · شغّل `C29_EXPORT_SAFETY_CHECK`، ونزّل الدفتر المكتمل منفصلًا، وارفع المحتويات النظيفة المدمجة فقط إلى المستودع العام الفارغ.
