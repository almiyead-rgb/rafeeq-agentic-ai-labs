...
1. Create an empty public GitHub repository during preflight, but leave it completely empty while you work from the saved Drive notebook through C29. `C23` and `C28` generate the two required learner reports automatically. · أنشئ مستودع GitHub عامًا وفارغًا أثناء التجهيز، لكن اتركه فارغًا تمامًا أثناء العمل من دفتر Drive حتى C29. تولد `C23` و`C28` تقريري المتدرب الإلزاميين تلقائيًا.
...
7. Upload only the combined clean contents to the still-empty public repository, then wait for **Actions → Learner submission quality** to turn green for the exact final commit. · ارفع المحتويات النظيفة المدمجة فقط إلى المستودع العام الذي ما زال فارغًا، ثم انتظر نجاح **Actions ← Learner submission quality** لنفس Commit النهائي.
